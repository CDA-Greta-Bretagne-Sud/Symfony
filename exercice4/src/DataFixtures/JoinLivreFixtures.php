...
class JoinLivreFixtures extends Fixture implements FixtureInterface, ContainerAwareInterface {
    ...
    public function load(ObjectManager $manager): void {
        ...
        $pers=$repPersonne->findOneBy(array('nom' =>'Macron')); $pers->addLivre($l3);
        $pers->addLivre($l2);
        $manager->persist($pers);

        $pers=$repPersonne->findOneBy(array('nom' =>'Hugo')); $pers->addLivre($l1);
        $pers->addLivre($l2);
        $manager->persist($pers);

        $pers=$repPersonne->findOneBy(array('nom' =>'Valjean')); $pers->addLivre($l3);
        $pers->addLivre($l2);
        $pers->addLivre($l1);
        $manager->persist($pers);
